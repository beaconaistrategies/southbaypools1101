// Standalone seed endpoint - creates golf pool schema and seeds sample data
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    try {
      // Create all enums and tables if they don't exist
      await client.query(`
      ...
      // Check if earnings pool already exists
      ...
      // Create the Masters 2026 Earnings Pool
      ...
      // Seed golfers by tier (realistic top-ranked golfers)
      const golfers = [
        ...
        { name: 'Ludvig Åberg', dgId: 25764, country: 'SWE', tier: 1, dgRank: 5 },
        ...
        { name: 'Joaquín Niemann', dgId: 20505, country: 'CHI', tier: 4, dgRank: 68 },
      ];

      // Insert golfers and collect their IDs by tier
      const golferIds: Record<number, string[]> = { 1: [], 2: [], 3: [], 4: [] };
      ...
      // Create sample entries (pool participants who picked golfers)
      const entries = [
      ...
      for (const e of entries) {
        await client.query(
          ...
          [poolId, e.name, e.email,
           golferIds[1][e.t1], golferIds[2][e.t2], golferIds[3][e.t3], golferIds[4][e.t4]]
        );
        entryCount++;
      }

      // Also seed the 2026 PGA Tour majors schedule
      const majors = [
