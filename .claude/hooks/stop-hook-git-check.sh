...
# Check if we're in a git repository first - bail if not
if ! git rev-parse --git-dir >/dev/null 2>&1; then
  exit 0
fi

# Check if stop hook is already active (recursion prevention)
# Use grep as fallback if jq is not available
if command -v jq >/dev/null 2>&1; then
  stop_hook_active=$(echo "$input" | jq -r '.stop_hook_active' 2>/dev/null || echo "")
else
  stop_hook_active=$(echo "$input" | grep -o '"stop_hook_active":\s*true' || echo "")
fi
if [[ "$stop_hook_active" = "true" ]] || [[ -n "$stop_hook_active" && "$stop_hook_active" != "" && "$stop_hook_active" != "false" && "$stop_hook_active" != "null" ]]; then
  exit 0
fi
